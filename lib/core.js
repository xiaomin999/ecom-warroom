...
(function () {
  ...
  /* ---------- 资料归档：把任意 AI 生成内容归入当前产品简报 ----------
     在任一模块生成结果后调用：ECOM.attachReport(type, title, content, nameOpt)
     - 自动并入当前选品简报（ECOM.brief）并持久化到选品库
     - 同一 type 多次调用会覆盖该份资料，其它 type 互不干扰
     - 无归属产品（无当前简报且无 nameOpt）时返回 null，不入档
     资料结构：brief.reports = { type: { title, content, ts }, ... } */
  /* 按名称定位目标简报：与当前简报同名则用当前，否则从选品库按名称找；都不存在则新建 */
  function targetBrief(name) {
    const cur = ECOM.getBrief() || {};
    if (cur && cur.name === name) return Object.assign({}, cur);
    const found = ECOM.briefLib.list().find(b => b.name === name);
    return found ? Object.assign({}, found) : { name: name };
  }
  // 写回：upsert 到选品库；若目标正是当前简报则同步内存与本地
  function persistBrief(merged) {
    ECOM.briefLib.upsert(merged);
    const cur = ECOM.getBrief() || {};
    if (cur && cur.name === merged.name) {
      ECOM.brief = merged;
      try { localStorage.setItem('ecom_brief_v1', JSON.stringify(merged)); } catch (e) {}
    }
    return merged;
  }

  ECOM.attachReport = function (type, title, content, nameOpt) {
    if (!content || !String(content).trim()) return null;
    const cur = ECOM.getBrief() || {};
    const name = (nameOpt && String(nameOpt).trim()) || (cur && cur.name) || '';
    if (!name) return null;
    const base = targetBrief(name);
    const reports = Object.assign({}, base.reports || {});
    reports[type] = { title: title || type, content: String(content), ts: Date.now() };
    return persistBrief(Object.assign({}, base, { name: name, reports: reports }));
  };

  /* 与 attachReport 同构，但写入 brief.forms[type]：归档「用户填写 / 测算的内容」 */
  ECOM.attachForm = function (type, title, content, nameOpt) {
    if (!content || !String(content).trim()) return null;
    const cur = ECOM.getBrief() || {};
    const name = (nameOpt && String(nameOpt).trim()) || (cur && cur.name) || '';
    if (!name) return null;
    const base = targetBrief(name);
    const forms = Object.assign({}, base.forms || {});
    forms[type] = { title: title || type, content: String(content), ts: Date.now() };
    return persistBrief(Object.assign({}, base, { name: name, forms: forms }));
  };

  /* 便捷：一次归档「AI 生成内容」与「填写内容」到同一产品资料库
     opts: { aiTitle, ai, formTitle, form, name } */
  ECOM.attachBoth = function (type, opts) {
    let res = null;
    if (opts.ai && String(opts.ai).trim()) res = ECOM.attachReport(type, opts.aiTitle || type, opts.ai, opts.name) || res;
    if (opts.form && String(opts.form).trim()) res = ECOM.attachForm(type, opts.formTitle || (type + '（输入）'), opts.form, opts.name) || res;
    return res;
  };
