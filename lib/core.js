...
(function () {
  ...
  ECOM.attachReport = function (type, title, content, nameOpt) {
    ...
    // 优先用调用方显式传入的产品名（内容模块里用户填的商品名），否则用当前简报的产品名
    const name = (nameOpt && String(nameOpt).trim()) || (cur && cur.name) || '';
    if (!name) return null;
    // 目标简报：与当前简报同产品则沿用当前（保留已有资料/字段）；否则按名称从选品库找回该产品的已有简报，避免混入别的产品资料
    let base = cur;
    if (nameOpt && String(nameOpt).trim() && cur.name && cur.name !== name) {
    ...
    return ECOM.setBrief({ name: name, reports: reports });
  };
