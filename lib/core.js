...
(function () {
  ...
  ECOM.llm = async function (messages, opts = {}) {
    ...
    const model = opts.model || s.model || 'gpt-4o-mini';
  ...
  ECOM.ui.resultCard = function (title) {
    ...
    wrap.innerHTML =
      '<div class="btn-row" style="justify-content:space-between;margin-bottom:10px">' +
        ...
        '<div class="btn-row">' +
          ...
          '<button class="secondary-btn dl-btn" style="padding:7px 12px;font-size:13px">下载</button>' +
        '</div>' +
      '</div>' +
    ...
    wrap.querySelector('.dl-btn').addEventListener('click', () => ECOM.ui.download(resultEl.dataset.file || 'result.txt', resultEl.dataset.raw || ''));
    wrap.set = set;
